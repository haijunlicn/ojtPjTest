...
/*	const linkInput = document.createElement("input")
	...
	// Fixed insertLink function based on user's code
	function insertLink() {
		const linkName = prompt("Enter link name:")
		const linkURL = prompt("Enter link URL:")

		if (linkName && linkURL) {
			// Ensure input is not empty and remove leading/trailing spaces
			const trimmedName = linkName.trim()
			const trimmedURL = linkURL.trim()

			if (!trimmedName || !trimmedURL) {
				alert("Invalid input. Please enter a valid name and URL.")
				return
			}

			// Create formatted entry (e.g., "Google|https://google.com")
			const linkEntry = `${trimmedName}|${trimmedURL}`
			...
			linkText.href = trimmedURL
			linkText.target = "_blank"
			linkText.textContent = trimmedName
			...
			// Create remove button
			const removeBtn = document.createElement("button")
			removeBtn.className = "remove-file btn btn-danger btn-sm ms-2"
			removeBtn.innerHTML = '<i class="fas fa-times"></i>'
			...
			removeBtn.addEventListener("click", (e) => {
				...
				linkInput.value = currentLinks.join(",")
				if (typeof updatePostButtonState === "function") {
					updatePostButtonState()
				}
			})
			...
			// Update post button state if the function exists
			if (typeof updatePostButtonState === "function") {
				updatePostButtonState()
			}
		}
	...
	/*function insertLink() {
	  ...
	  if (linkName && linkURL) {
		...
		// Create list item with Font Awesome icon
		const listItem = document.createElement("li")
